      ...
      let
        ...
        filePath =
          if hasSlash then
            ...
            in
            if nestedExists.success && nestedExists.value then "${name}/${name}.nix" else "${name}.nix";
      in
      {
      ...
      customNeovim = inputs.nvf.lib.neovimConfiguration {
        ...
        modules = allModules ++ [
          # Pass the nixpkgs packages to various modules
          {
            ...
            # nvim-treesitter (required by lzn-auto-require from nvchad-ui)
            config.vim.extraPlugins.nvim-treesitter.package = pkgs.vimPlugins.nvim-treesitter;
