    ...
    { class, aspect-chain }:
    {
      nixos =
        ...
        let
          ...
          sshKey =
            if sshKeyPathExists.success && sshKeyPathExists.value then
              lib.strings.trim (builtins.readFile defaultSshKeyPath)
            else
              null;
        in
        {
          ...
          options.deployment = {
            enable = lib.mkEnableOption "deployment configuration" // {
              default = true;
            };
          ...
          config = lib.mkIf config.deployment.enable {
            ...
            users.users.${config.deployment.username} = {
              isNormalUser = true;
              extraGroups = [ "wheel" ];
              openssh.authorizedKeys.keys = lib.mkIf (config.deployment.sshKey != null) [
                config.deployment.sshKey
              ];
            };
