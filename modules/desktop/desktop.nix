...
# Intelligently sets up desktop environment, display manager, and common settings
{
  ...
  FTS.desktop.__functor = _self: {
    ...
    envDefault =
      if environment != null
      then environment.default or null
      else null;
    ...
    bootloaderDefault =
      if bootloader != null
      then bootloader.default or null
      else null;
    bootloaderGrubConfig =
      if bootloader != null
      then bootloader.grub or {}
      else {};
    bootloaderSystemdConfig =
      if bootloader != null
      then bootloader.systemd or {}
      else {};
    ...
    _ =
      if envDefault != null && !(builtins.elem envDefault availableEnvs)
      then throw "desktop: unknown environment '${envDefault}'. Available: ${builtins.concatStringsSep ", " availableEnvs}"
      else if bootloaderDefault != null && !(builtins.elem bootloaderDefault availableBootloaders)
      then throw "desktop: unknown bootloader '${bootloaderDefault}'. Available: ${builtins.concatStringsSep ", " availableBootloaders}"
      else null;
    ...
    dmAuto =
      ...
      then displayManager.auto or true
      else true;
    dmType =
      if displayManager != null
      then displayManager.type or null
      else null;

    # Auto-select display manager based on desktop environment
    autoDisplayManager =
      if !dmAuto && dmType != null
      then dmType
      else if dmType != null
      then dmType
      else if envDefault == "gnome"
      then "gdm"
      else if envDefault == "kde"
      then "sddm"
      else if envDefault == "hyprland"
      then "gdm"
      else if envDefault == "xfce"
      then "lightdm"
      else null;
    ...
    dmIncludes = lib.optionals (autoDisplayManager != null) [
      ...
      (
        if autoDisplayManager == "gdm"
        then FTS.gdm
        else if autoDisplayManager == "sddm"
        then FTS.sddm
        else if autoDisplayManager == "lightdm"
        then FTS.lightdm
