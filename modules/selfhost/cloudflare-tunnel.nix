      ...
      nixos =
        ...
        let
          ...
          getServerName =
            ...
            in
            if serverName != null && serverName != "" then serverName else hostKey;

          # Clean domain names (remove http:// or https:// prefix)
          cleanDomain =
            host:
            if lib.hasPrefix "http://" host then
              lib.removePrefix "http://" host
            else if lib.hasPrefix "https://" host then
              lib.removePrefix "https://" host
            else
              host;
          ...
          getDestination =
            ...
            let
              ...
              proxyPassFromConfig = if proxyPassMatch != null then builtins.head proxyPassMatch else null;

              # Use whichever we found, or default to https://localhost for direct nginx hosts
              rawProxyPass =
                if proxyPassAttr != null then
                  proxyPassAttr
                else if proxyPassFromConfig != null then
                  proxyPassFromConfig
                else
                  "https://localhost"; # Default for hosts served directly by nginx (like Nextcloud, Jellyfin)

              # Strip trailing slash if present (cloudflared doesn't like paths)
              proxyPass =
                if rawProxyPass != null && lib.hasSuffix "/" rawProxyPass then
                  lib.removeSuffix "/" rawProxyPass
                else
                  rawProxyPass;
            in
            proxyPass;
          ...
          shouldExclude = domain: excludeJDownloader && lib.hasInfix "jdownloader" (lib.toLower domain);

          # Check if a service URL is valid for cloudflared (no unix sockets)
          isValidService =
            service:
            service != null && !(lib.hasPrefix "http://unix:" service) && !(lib.hasPrefix "unix:" service);

          # Build ingress rules as an array
          autoDetectedRules = lib.foldl' (
            acc: hostKey:
            let
            ...
            if shouldExclude cleanedDomain || destination == null || !isValidService destination then
              acc
            else
              acc
              ++ [
        ...
        in
        {
          ...
          systemd.services."cloudflared-tunnel-${tunnelId}" = {
            ...
            after = [
              ...
              "cloudflared-credentials-${tunnelId}.service"
            ];
          ...
          systemd.services."cloudflared-dns-route-${tunnelId}" = lib.mkIf autoRouteDNS {
            ...
            script =
              let
                ...
                domainNames = builtins.filter (d: d != null) (
                  map (
                    hostKey:
                    let
                      serverName = getServerName hostKey;
                    in
                    if serverName != null then cleanDomain serverName else null
                  ) nginxHosts
                );
